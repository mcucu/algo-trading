...
bool HTF_Uptrend()
{
   return iClose(_Symbol,HTF,1) >
          iMA(_Symbol,HTF,EMA_Mid,0,MODE_EMA,PRICE_CLOSE);
}
bool HTF_Downtrend()
{
   return iClose(_Symbol,HTF,1) <
...
{
   gRangeHigh = iHigh(_Symbol,_Period,1);
   gRangeLow  = iLow(_Symbol,_Period,1);
...
void TradeTrendImpulse()
{
   if(!(BUF(hEMA20,0,1)>BUF(hEMA50,0,1) &&
        BUF(hEMA50,0,1)>BUF(hEMA200,0,1)))
   {
      LogSkip("Impulse EMA invalid");
      return;
   }
