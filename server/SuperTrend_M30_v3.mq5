...
// --- Exhaustion Filter
input double ExhaustionATR = 2.0;   // XAU/XAG = 2.0
...
// --- Slope Filter
input double MinSTSlopeATR = 0.3;

// --- Panic Exit
input double PanicATR = 1.5;

// --- Direction Lock
input int DirectionLockLoss = 2;
input int DirectionLockHours = 8;
...
int buyLossCount = 0;
int sellLossCount = 0;
datetime lockBuyUntil = 0;
datetime lockSellUntil = 0;
...
int OnInit()
{
   ...
   if(hATR==INVALID_HANDLE || hEMA50==INVALID_HANDLE || hBB==INVALID_HANDLE)
      return INIT_FAILED;
...
bool ExhaustionOK()
{
   ...
   double dist = MathAbs(iClose(_Symbol,_Period,1)-ema50);
   return dist <= ExhaustionATR * atr;
}

// --- BB Expansion
bool BBExpansionOK(bool isBuy)
{
   double upper = BUF(hBB,1,1);
   double lower = BUF(hBB,2,1);
   double width1 = upper-lower;

   double upperPrev = BUF(hBB,1,3);
   double lowerPrev = BUF(hBB,2,3);
   double width2 = upperPrev-lowerPrev;

   if(width1 <= width2) return true;

   if(isBuy && iClose(_Symbol,_Period,1)>upper) return false;
   if(!isBuy && iClose(_Symbol,_Period,1)<lower) return false;
...
{
   if(isBuy && TimeCurrent()<lockBuyUntil) return false;
   if(!isBuy && TimeCurrent()<lockSellUntil) return false;
   return true;
}
...
bool GetSuperTrend(ENUM_TIMEFRAMES tf,double &st,bool &bull)
{
   ...
   int sthATR=iATR(_Symbol,tf,ST_ATR_Period);
   double atrBuf[]; if(CopyBuffer(sthATR,0,0,1,atrBuf)<=0) return false;
   atr=atrBuf[0]; IndicatorRelease(sthATR);
   double h[],l[],c[];
